...
// When the user scrolls down 20px from the top of the document, show the button
	...
	function liveRate() {	   
		var evoaiAPI = "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=BTC&tsyms=USD";
		$.getJSON( evoaiAPI, {
			tags: "evoai network",
			tagmode: "any",
			format: "json"
		...
		.done(function( data ) { 
			var str_val = JSON.stringify(data);
			var json_arr = jQuery.parseJSON(str_val);
			
			var val = '';
			
			var CHANGE_PCTDAY = JSON.stringify(data.RAW.BTC.USD.CHANGEPCTDAY);
	   ...
	   /*
		$.ajax({
			...
			var CHANGE_PCTDAY = JSON.stringify(result.RAW.BTC.USD.CHANGEPCTDAY);
			var img_btc = JSON.stringify(json_arr.RAW.BTC.USD.LASTUPDATE);
			var CHANGE_PCTDAY1 = Number (CHANGE_PCTDAY).toFixed(2);
			val += ' <div class="pull-left price-grp"><div class="curr"><span class="cname">BTC/USD</span><span> <span class="rvalue"> '+CHANGE_PCTDAY1+'</span><span class="cper">%</span></span></div><div class="realR"><strong> '+JSON.stringify(json_arr.RAW.BTC.USD.PRICE)+'</strong> USD</div><div class="cvol">Vol:<strong> '+JSON.stringify(json_arr.RAW.BTC.USD.LASTVOLUME)+'</strong></div></div><div class="pull-left img-grp"><img class="spark-img" src="https://images.cryptocompare.com/sparkchart/BTC/USD/latest.png?ts='+img_btc+'"></div>';
			$('.live-rate').html(val);
	...
	function liveRate_BABB() {
		...
		/*OP*/
		var evoaiAPI = "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=BAB&tsyms=USD";
		$.getJSON( evoaiAPI, {
			tags: "evoai network",
			tagmode: "any",
			format: "json"
	...
	function liveRate_ETN() {
		...
		var evoaiAPI = "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=ETN&tsyms=USD";
		$.getJSON( evoaiAPI, {
			tags: "evoai network",
			tagmode: "any",
			format: "json"
	...
	function liveRate_TRON() {
		...
		var evoaiAPI = "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=TRON&tsyms=USD";
		$.getJSON( evoaiAPI, {
			tags: "evoai network",
			tagmode: "any",
			format: "json"
	...
	function liveRate_eth() {
		...
		var evoaiAPI = "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=ETH&tsyms=USD";
		$.getJSON( evoaiAPI, {
			tags: "evoai network",
			tagmode: "any",
			format: "json"
	...
	function liveRate_ltc() {
		...
		var evoaiAPI = "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=LTC&tsyms=USD";
		$.getJSON( evoaiAPI, {
			tags: "evoai network",
			tagmode: "any",
			format: "json"
	...
	function liveRate_bch() {
		...
		var evoaiAPI = "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=BCH&tsyms=USD";
		$.getJSON( evoaiAPI, {
			tags: "evoai network",
			tagmode: "any",
			format: "json"
